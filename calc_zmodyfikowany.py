...
class TooHighTempException(Exception): pass


def generate_data():
    iso140_08 = Material(name="ISO 140-0.8", max_temp=1400, coeff_400=0.27, coeff_800=0.31, coeff_1000=0.33, coeff_1200=0.36)
    iso125_05 = Material(name="ISO 125-0.5", max_temp=1250, coeff_400=0.14, coeff_600=0.16, coeff_800=0.18, coeff_1000=0.2)
    iso85_06 = Material(name="ISO 185-0.6", max_temp=850, coeff_400=0.14, coeff_600=0.16, coeff_800=0.19)
    microporus_1000 = Material(name='Microporous ISO 1000', max_temp=1000, coeff_200=0.022, coeff_400=0.025, coeff_600=0.035, coeff_800=0.044)
    microporus_1200 = Material(name='Microporous ISO 1200', max_temp=1200, coeff_200=0.029, coeff_400=0.033, coeff_600=0.039, coeff_800=0.044)
...
def main():
    ...
    with db_session:
        ...
        for name, thickness in wall_config:
            ...
            layer_coeff = material.coeff_a * (temp ** 2) + material.coeff_b * temp + material.coeff_c
            print(f'Temperatura na warstwie {name} jest rowna {temp}')
            temp = temp - ((thickness * Q) / layer_coeff)


    if temp > TEMP_END:
        print(f"Mamy problem, temp końcowa: {round(temp, 2)}*C jest większa niż zakładana {TEMP_END}*C")
    else:
